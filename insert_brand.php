...
if (isset($_POST['submit'])) {
    ...
    if (!empty($brand_name)) {
        // Prepare and execute statement to check for existing brand
        $check = $conn->prepare("SELECT * FROM brands WHERE brand_name=?");
        ...
        if ($result->num_rows > 0) {
            ...
            // Prepare and execute statement to insert new brand
            $insert = $conn->prepare("INSERT INTO brands (brand_name) VALUES (?)");
            $insert->bind_param("s", $brand_name);
            if ($insert->execute()) {
...
<html lang="en" class="light"> <head>
    <meta charset="UTF-8">
    <title>Insert Brand - ShadaShop Admin</title>
...
<div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 transform transition-all duration-300">
    <h2 class="text-3xl font-bold text-center mb-6 text-blue-600 dark:text-blue-400">Insert New Brand</h2>
    ...
    <form method="post" class="space-y-5">
        <div>
            ...
            <input type="text" name="brand_name" id="brand_name" class="mt-1 block w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Enter Brand Name" required>
        </div>
        <button type="submit" name="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-semibold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
            Insert Brand
        </button>
