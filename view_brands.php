...
if (isset($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['error_message'] = "Error deleting brand: " . $stmt->error;
    }
    $stmt->close();
...
<html lang="en" class="light"> <head>
    <meta charset="UTF-8">
    <title>View Brands - ShadaShop Admin</title>
...
<div class="w-full max-w-4xl bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 transform transition-all duration-300">
    <h2 class="text-3xl font-bold text-center mb-6 text-blue-600 dark:text-blue-400">View Brands</h2>
    ...
    <div class="overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow-md mb-6">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-200 dark:bg-gray-700">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Brand Name</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <?php while($row = $brands->fetch_assoc()): ?>
                        <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100"><?= $row['brand_id']; ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300"><?= htmlspecialchars($row['brand_name']); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="?delete=<?= $row['brand_id']; ?>" 
        ...
        <?php else: ?>
            <div class="bg-blue-100 dark:bg-blue-900 border-l-4 border-blue-500 dark:border-blue-400 text-blue-700 dark:text-blue-200 p-4 rounded-lg my-6" role="alert">
                <h4 class="font-bold text-xl mb-2">No Brands Found!</h4>
                <p>There are no brands to display yet. Consider adding some from the admin panel.</p>
            </div>
        <?php endif; ?>
    </div>
